...
Shader "Raymarching/Simple/BasicFix2"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "./Include/Fractals.cginc"
...
#define STEP_SIZE 0.01
			...
			inline float distanceFunction(float3 pos) 
			{
				...
				//float radius = 0.4;
				//return blend(Box(pos, 0.4), Torus(pos, float2(0.5, 0.4))  , (_SinTime[3] + 1.) / 2 );
				//return RecursiveTetrahedron(pos, 20); 
				float f = 0;
				//f = Box(pos, 0.2) ;
				//f = Box(Repeat(pos, 0.5), 0.1);
				//return distance(rpos, centre) - radius;

				//perhaps try intersecting with a "scene box" to define extents of object

				//god painting boxes
				f = smin(Box(pos + .1 + _SinTime[3]/4, 0.2), Box(pos - .1 - _SinTime[3]/2, 0.2), 0.5);
				f = smin(Box(pos + .1 + _SinTime[3] / 5, 0.2), Box(pos - .1 - _SinTime[3] / 2.5, 0.2), 0.5);

				//Clip to box
				//if (_isClipped && IsOutsideBox(pos, 1)) return _MinDistance * 2;
				if (!all(max(1.0 * 0.5 - abs(pos), 0.0))) return _MinDistance*2;
			...
			{ 
				//LIGHTING FLIPS FROM SOME ANGLES, ALAN ZUCCONI SAID THERE WAS A TYPO IN THE COMMENTS, CHECK PART 3 BLOG POST. TODO: FIX
				fixed3 lightDir = -_WorldSpaceLightPos0.xyz;	// Light direction, flips randomly! Also, the minus in front isn't supposed to be there, but doesn't change anything until you fix the flipping problem - it just flips from the other direction when the minus is there.
			...
			fixed4 getRaymarchShaded(float3 position, float3 direction)
			{
				for (int i = 0; i < _Steps; i++)
				{
					...
					{
						//if (IsOutsideSphere(position, 0.7)) break; //only render if in bounds

						//if (IsOutsideBox(position, 1)) break; 

						return renderVolumeSurface(position, direction);
						//return renderVolumeSurface(position, direction);
					}

					//Are we past the extents of the object space?
					if (_isClipped > 0)
					{
						IsInnerSphere2(position, 0.5);
						IsInnerObject(position);
						//if (!IsInnerObject(position)) discard;
					} 
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 color = getRaymarchShaded(usedPosition, viewDirection);

				//Extremely Basic AO
				//color *= 1-getRaymarchDistance(worldPosition, viewDirection);
